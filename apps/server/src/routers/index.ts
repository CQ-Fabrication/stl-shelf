...
export const appRouter = {
	...
	getModel: publicProcedure
		...
		.handler(async ({ input }) => {
			await initializeServices();
			const model = await fsService.getModel(input.id);
			if (!model) {
	...
	getModelFile: publicProcedure
		...
		.handler(async ({ input }) => {
			await initializeServices();
			const filePath = join(dataDir, input.modelId, input.version, input.filename);
	...
	updateModelMetadata: publicProcedure
		...
		.handler(async ({ input }) => {
			...
			await fsService.saveMetadata(input.modelId, input.version || null, input.metadata);
	...
	getRepositoryStatus: publicProcedure
		.handler(async () => {
			await initializeServices();
