...
app.post("/upload", async (c) => {
	try {
		...
		await gitService.commitModelUpload(modelId, version, `Add ${name} ${version}`);
...
if (process.env.NODE_ENV === 'production') {
	...
	app.use("/*", serveStatic({ 
		...
		onNotFound: (path, c) => {
			...
			if (!path.startsWith("/rpc") && !path.startsWith("/upload") && !path.startsWith("/files") && !path.startsWith("/thumbnails")) {
