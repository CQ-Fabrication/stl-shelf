  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      ...
      if (new3DFiles.length > 0) {
        ...
        if (!form.state.values.name && new3DFiles.length > 0) {
          const fileName = new3DFiles[0].file.name;
      ...
      if (newImageFile) {
        ...
        uploadModalActions.updateFormData("previewImage", newImageFile);
  ...
  const handleSubmit = async () => {
    ...
    if (files.length > 5) {
      toast.error("Too many files", {
        description: "Maximum 5 files allowed per model",
      });
      return;
    }
