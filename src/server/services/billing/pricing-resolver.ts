...
/**
 * Server-only pricing resolution: live Polar fetches, last-known-good caches,
 * and BetterStack failure reporting.
 *
 * MUST NOT be imported from client code (it evaluates `env` and logging).
 * The server fns in src/server/functions/pricing.ts are the only entry point;
 * that module exports nothing but server fns and types so the client bundle
 * can strip this whole graph.
 */
...
function getPaidTierProductId(
  slug: Exclude<TierSlug, "free">,
...
async function buildPaidTier(slug: Exclude<TierSlug, "free">): Promise<PricingTier> {
  ...
  const snapshots = await Promise.all(
    BILLING_INTERVALS.map(async (interval) => {
      const productId = getPaidTierProductId(slug, interval);
...
export async function loadPublicPricing(): Promise<PublicPricingResponse> {
  const now = Date.now();
...
export async function loadAddonPricing(): Promise<AddonPricingResponse> {
  const now = Date.now();
