...
export function Particles({
  ...
  // Canvas animation setup - functions are intentionally not in deps as they use refs
  useEffect(() => {
    if (!isMounted) return;
    if (canvasRef.current) {
  ...
  // Refresh canvas when prop changes
  useEffect(() => {
    initCanvas();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refresh]);
  ...
  if (!isMounted) {
    return <div className={cn("pointer-events-none", className)} aria-hidden="true" {...props} />;
  }
