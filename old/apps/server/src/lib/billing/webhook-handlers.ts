...
export const handleSubscriptionCreated = async (
  ...
  console.log(
    `[Polar Webhook] Subscription created: ${tier} (status: ${subscription.status})`
  );
};
 ...
 */
export const handleCustomerStateChanged = async (
  ...
  if (activeSubscriptions.length > 0) {
    // Use the first active subscription (should only have one in most cases)
    ...
    console.log(
      "[Polar Webhook] Customer state synced: free tier (no active subscription)"
    );
  }
};
